...
- M81 historical validation command and data readiness is complete:
  ...
  coverage is short.
- M82 technical validation reports and conservative gate work is complete:
  `make validate-technical-v2` now writes an operator Markdown report under
  `docs/reports/technical_validation/<run_id>.md`, technical-agent predictive
  JSON/CSV/Markdown artifacts, full-system backtest JSON/CSV/Markdown
  artifacts, an expanded profile comparison matrix, and a conservative
  `promotion_gate.json` that recommends `promote`, `keep_opt_in`, or `defer`
  without promoting v2.
- M83 official index, sector, and India VIX ingestion work is complete:
  `official_index_candles` stores official benchmark, sector-index, and
  volatility-index OHLC history with source, timeframe, identifier, family, raw
  source row, and `data_available_time`; repository as-of queries filter by
  availability time to prevent lookahead; `make import-official-index-data`
  imports official CSVs; and `make check-official-index-readiness` produces
  explicit v2B readiness failures for missing benchmark, sector, or India VIX
  history without wiring official data into v2A scoring.
- M84 official microstructure ingestion work is complete:
  `official_security_microstructure` stores security-wise delivery,
  circuit/price-band, average trade value, turnover, impact-cost, and explicit
  implementability source-kind labels with as-of repository access and
  readiness checks for v2B use.
- M85 official-data technical profile work is complete: the opt-in
...
- Assumptions made: M85 should remain additive and opt-in only; v2B should
  reuse the v2A OHLCV feature suite and add official-data context rather than
  changing `technical_ohlcv_v2`; official sector-relative evidence should be
...
- Mocks created: Deterministic in-test official benchmark, sector-index, India
  ...
  `OfficialTechnicalContext` fixtures in
  `tests/unit/test_strategy_ranking.py`.
- Mocks used: Project-local Postgres test fixtures and deterministic in-memory
  official context objects only; no external official CSVs or live services
  were used.
- Verification: `uv run python -m compileall` on touched modules and tests;
  `uv run pytest tests/unit/test_official_technical_context.py tests/unit/test_technical_signal_service.py tests/unit/test_strategy_ranking.py tests/unit/test_graph_backtesting.py tests/unit/test_money_management.py tests/unit/test_paper_runs.py`
  passed with 74 tests; `uv run pytest tests/unit/test_config.py
...
- Assumptions made: M83 should create official-data ingestion and readiness
  contracts only, leaving `technical_ohlcv_v2`, `graph_aware_score_v2`,
  `technical_rule_v1`, `graph_aware_score_v1`, `make validate-technical-v2`,
  and `make paper-loop-kite` scoring behavior unchanged until M85; broad
  benchmark and India VIX readiness should default to `NIFTY_50` and
  `INDIA_VIX`, while sector-index readiness is explicit through
  `OFFICIAL_INDEX_SECTOR_SYMBOLS` until a later symbol-to-sector mapping exists;
  official CSV rows should keep source-row metadata and use
  `data_available_time` as the no-lookahead gate.
- Mocks created: `tests/unit/test_official_index_data.py` creates deterministic
  benchmark, sector-index, and India VIX fixtures plus a temporary official
  index CSV.
- Mocks used: The focused tests use the project-local Postgres test fixture and
  deterministic in-test official index rows only; no unofficial local
  market/sector proxies are used for scoring.
- Verification: `uv run pytest tests/unit/test_official_index_data.py` passed
  with 4 tests; `uv run pytest tests/unit/test_config.py
  tests/unit/test_dashboard_observability.py
  tests/unit/test_official_index_data.py` passed with 33 tests; `make migrate`
  passed; `make lint` passed; and `uv run pytest tests/unit/test_migrations.py`
  passed with 3 tests. Approval-rule cleanup found no accidental
  Taurus-specific global approvals after the `# END MY CUSTOM ADDITION` marker;
  the gitignored project-local `.codex/rules/default.rules` was updated locally
  with the two M83 Make targets.

### M82 Completion Summary

- Assumptions made: M82 should extend the existing validation command instead
  of adding a second runner; report generation should remain deterministic from
  validation artifacts and local candles, so generated report files are ignored
  as run output; the backtest layer does not execute the live planner
  allocation ledger, so full-system allocation behavior is reported as an
  explicit backtest proxy using raw strategy score distribution, calibrated
  allocation score components, selected/fill counts, cash utilization, and
  inferred sizing failures; insufficient local history should still exit zero
  by default and produce a conservative `defer` report.
- Mocks created: Added a focused long-horizon `VGA`/`VGB` validation fixture in
  `tests/unit/test_graph_backtesting.py` so 5d, 21d, and 63d predictive checks
  have real future-return labels; added `tests/unit/test_backtesting.py` so the
  documented verification command has a concrete backtesting test target.
- Mocks used: Existing graph backtest fixture helpers, deterministic `_candles()`
  test candles, temp artifact/report roots, and the existing project-local
  Postgres-backed test settings.
- Verification: `make validate-technical-v2` passed and wrote
  `artifacts/technical_validation/techval-7114d6243f898127/` plus ignored
  operator report
  `docs/reports/technical_validation/techval-7114d6243f898127.md`; current
  local `configs/market_data/nifty_50_shariah.yaml` coverage remained
  insufficient at 282 common candles versus the required window, so the
  conservative recommendation was `defer` with deeper import guidance
  `TAURUS_MARKET_DATA_LOOKBACK_DAYS=1434 make import-kite-candles`.
  `uv run pytest tests/unit/test_backtesting.py
  tests/unit/test_graph_backtesting.py tests/unit/test_paper_runs.py` passed
  with 46 tests. Approval-rule cleanup found no accidental Taurus-specific
  global approvals after the `# END MY CUSTOM ADDITION` marker.
