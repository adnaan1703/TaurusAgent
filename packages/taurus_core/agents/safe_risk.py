...
class SafeRiskAgent:
    ...
    def run(self, *, proposal: TraderProposal, settings: Settings) -> RiskPersonaReview:
        ...
        half_cap = (max_position / Decimal("2")).quantize(SCORE_QUANT)
        if proposal.action in {"HOLD", "NO_TRADE", "REDUCE", "EXIT"}:
            ...
            key_points = [
                f"Conservative view avoids exposure for action {proposal.action}."
            ]
            ...
            recommendation = (
                "reduce" if proposal.requested_position_pct_nav > half_cap else "allow"
            )
