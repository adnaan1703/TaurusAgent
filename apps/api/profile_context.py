...
def active_profile(
    ...
    *,
    ...
    selected_profile_id = _validated_profile_id(profile_id or settings.effective_profile_id)
    model = TaurusProfileRepository(session).get_profile(selected_profile_id)
    if model is None:
        raise HTTPException(status_code=404, detail=f"Profile {selected_profile_id} not found.")
    if model.status != "ACTIVE":
        raise HTTPException(status_code=422, detail=f"Profile {selected_profile_id} is archived.")
    return model
...
def profile_for_run(
    ...
    *,
    ...
    run_profile_id = _validated_profile_id(run.portfolio_id or settings.effective_profile_id)
    if profile_id is not None:
        requested = active_profile(session, settings, profile_id=profile_id)
        if requested.profile_id != run_profile_id:
            raise HTTPException(status_code=404, detail="Paper run not found for selected profile.")
        return requested

    model = TaurusProfileRepository(session).get_profile(run_profile_id)
    if model is None:
        raise HTTPException(status_code=404, detail=f"Profile {run_profile_id} not found.")
    return model
