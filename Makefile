...
paper-loop-kite:
	DATABASE_URL="$(DATABASE_URL)" TAURUS_MARKET_DATA_PROVIDER=kite TAURUS_ENABLED_ANALYSTS=technical,graph TAURUS_GRAPH_ENABLED=true TAURUS_GRAPH_RISK_ENABLED=true TAURUS_PAPER_ANALYSIS_SCOPE=full_universe TAURUS_PAPER_EXECUTION_SCOPE=allocated_only STRATEGY=configs/strategies/graph_aware_score_v1.yaml SYMBOL="$(PAPER_LOOP_KITE_SYMBOL)" SYMBOLS="$(PAPER_LOOP_KITE_SYMBOLS)" PAPER_LOOP_ITERATIONS="$(PAPER_LOOP_ITERATIONS)" PAPER_LOOP_INTERVAL_SECONDS="$(PAPER_LOOP_INTERVAL_SECONDS)" PYTHONPATH=packages:. $(UV) run python scripts/run_paper_loop.py
