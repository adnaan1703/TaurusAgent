...
from taurus_core.data.providers.factory import (
    REAL_MARKET_DATA_PROVIDERS,
...
def test_provider_factory_lists_supported_real_providers_for_unsupported_provider() -> (
    None
):
...
def test_kite_preflight_rejects_legacy_mock_candles() -> None:
    ...
    with session_factory() as session:
        with pytest.raises(
            MarketDataProviderError, match="legacy mock_market_data candle rows"
        ):
