...
def build_login_url(
    settings: Settings | None = None, *, client: KiteAuthClient | None = None
) -> str:
    settings = settings or get_settings()
    if not settings.kite_api_key:
        raise MarketDataProviderError(
            "KITE_API_KEY is required to build the Kite login URL."
        )
...
def exchange_request_token(
    ...
    *,
    ...
    data = client.generate_session(
        normalized_token, api_secret=settings.kite_api_secret
    )
    access_token = data.get("access_token")
    if not isinstance(access_token, str) or not access_token.strip():
        raise MarketDataProviderError(
            "Kite generate_session response did not include access_token."
        )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser(
        "login-url", help="Print the Kite login URL for the configured API key."
    )
    ...
    exchange.add_argument(
        "--request-token", default=os.environ.get("KITE_REQUEST_TOKEN", "")
    )
